...
import { Transaction, TransactionDocument, TransactionType, TransactionPurpose, TransactionStatus } from '../../schemas/transaction.schema';
...
export class WalletsService {
  ...
  async getOrCreateWallet(userId: string): Promise<WalletDocument> {
    ...
    if (!wallet) {
      wallet = await this.walletModel.create({ user: new Types.ObjectId(userId), balance: 0 });
    }
    return wallet;
  }
  ...
  async getTransactions(userId: string, page = 1, limit = 20) {
    ...
    const [transactions, total] = await Promise.all([
      this.transactionModel
        ...
        .limit(limit)
        .lean(),
      this.transactionModel.countDocuments({ user: new Types.ObjectId(userId) }),
    ]);

    return { transactions, total, page, pages: Math.ceil(total / limit) };
  }

  async creditWallet(userId: string, amount: number, purpose: TransactionPurpose, reference: string, description: string, metadata?: any) {
  ...
  async debitWallet(userId: string, amount: number, purpose: TransactionPurpose, reference: string, description: string, metadata?: any) {
    const wallet = await this.getOrCreateWallet(userId);
  ...
  async initializeFunding(userId: string, amount: number, email: string, callbackUrl?: string) {
  ...
  async syncEarnings(userId: string) {
    ...
    for (const order of confirmedOrders) {
      ...
      const amountToCredit = (order as any).totalAmount - (order as any).commissionAmount;
    ...
    for (const earning of availableEarnings) {
      const reference = `order_a_${(earning as any).order}`;
      const existing = await this.transactionModel.findOne({ reference });
      if (!existing) {
        await this.creditWallet(
          userId,
          (earning as any).amount,
          TransactionPurpose.EARNING,
          reference,
          `Sync: Commission for order #${(earning as any).order.toString().slice(-6).toUpperCase()}`
        );
        creditedCount++;
        creditedAmount += (earning as any).amount;
      }
    }
